...
describe('decks_get_deck_by_id_handler', () => {
  ...
  it('should get a public deck without authentication', async () => {
    ...
    expect(result.name).toBe('Public Deck');
  ...
  it('should get an unlisted deck without authentication', async () => {
    const deck = await create_handler.execute({
      user_id: 'user-1',
      name: 'Unlisted Deck',
      front_language: 'es',
  ...
  it('should return correct deck properties', async () => {
    ...
    expect(result.front_language).toBe('fr');
